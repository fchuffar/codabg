...
nb_genes = 1000
df = data.frame(
  sex=c("M", "F")[round(runif(600))+1],
  age = round((runif(600)+1)*40),
  histology=c("ADK", "SQC")[round(runif(600))+1]
)
expr = (
  matrix(rep(rnorm(nb_genes, 0, 0.2), each=nb_obs), nrow=nb_obs) * (as.numeric(df$histology) - 1) +
  matrix(rep(rnorm(nb_genes, 0, 0.2), each=nb_obs), nrow=nb_obs) * (as.numeric(df$sex) - 1)      +
...
data_truth = data_test$histology
names(data_truth) = rownames(data_test)
print(data_truth)

data_test[1:nrow(data_test),"histology"] = NA
head(data_test[, 1:10])
dim(data_test)
print(data_test$histology)

head(data_train[, 1:10])
dim(data_train)
print(data_train$histology)
