...
```{r label = "GENERATOR - Knitr configuration", child = "Common_knitr_configuration.Rmd", include = FALSE}
```
```{r label = "GENERATOR - Knitr configuration (more)", include = FALSE}
...
if ( configuration$loadConfigurationFile ) {
    if ( file.exists( "configuration.rds" ) ) {
        configuration <- readRDS(file = "configuration.rds")
    } else {
...
Save configuration file : TRUE / FALSE. If TRUE, it will save the configuration into the file `configuration.rds`.
```{r label = "GENERATOR - Save configuration file"}
if ( is.null(x = configuration$saveConfigurationFile) ) {
    configuration$saveConfigurationFile <- readline(prompt = "Save configuration file (format : TRUE / FALSE) [default : FALSE] : ")
    configuration$saveConfigurationFile <- as.logical(x = configuration$saveConfigurationFile)
    if ( is.na(x = configuration$saveConfigurationFile) ) {
        configuration$saveConfigurationFile <- TRUE
    }
}
print(x = configuration$saveConfigurationFile)
if ( configuration$saveConfigurationFile ) {
...
for ( phase_idx in seq_len(length.out = configuration$phase_number ) ) {
    ...
    if ( !useFile ) {
        files <- readline(prompt = paste("Phase ", phase_idx, " : reference data : files to add inside the zip file '", configurationYaml$phases[[ phase_idx_c ]]$reference_data, "' (format : comma separated filenames) [default : reference_data.rds] : ", sep = "") )
            if ( files == "" ) {
                files <- "reference_data.rds"
            }
        files <- unlist(x = strsplit(x = files, split = ",") )
        zip::zipr(
            zipfile = zipName
...
# Rendering of a web site to previsualize the competition
...
`rmarkdown::render_site(input = "GENERATOR.Rmd", envir = new.env( ) )`
