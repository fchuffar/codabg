...
```{r README - Knitr configuration, child = "Common_knitr_configuration.Rmd", include = FALSE}
```

# TODO
- If you use R on Windows, you have to use the function "zip::zip" instead of the function "zip" to create a zip archive.

# Template for CodaLab competition bundles
...
```{r README - Package dependencies, echo = FALSE}
...
if ( auto_install ) {
    ...
    for (package in c("R.utils", "rmarkdown", "registry", "pkgmaker", "rngtools", "NMF") ) { ## the packages "registry", "pkgmaker", "rngtools", "NMF" have to be removed : used only for the current test.
...
```{r README - Command line arguments, include = FALSE}
# User variables :
...
```{r README - Challenge name, echo = FALSE, comment = ""}
if ( is.null(x = configuration$name) ) {
    configuration$name <- readline(prompt = "Name of the challenge (format : string) [default : template] : ")
...
## III. Competition properties

### 1. General Information using the YAML file
- `title` : string between double quotes
```{r README - Title, echo = FALSE}
if ( is.null(x = configuration$title) ) {
    configurationYaml$title <- readline(prompt = "Title (format : string) [default = TEMPLATE : title.] : ")
}
if ( configurationYaml$title == "" ) {
    configurationYaml$title <- "TEMPLATE : title."
}
print(x = configurationYaml$title)
```

- `description` : string between double quotes (max 256 characters)
```{r README - Description, echo = FALSE}
if ( is.null(x = configuration$description) ) {
    configurationYaml$description <- readline(prompt = "Description (format : string, max 256 characters) [default = TEMPLATE : description.] : ")
}
if ( configurationYaml$description == "" ) {
    configurationYaml$description <- "TEMPLATE : description."
}
...
```{r README - Disallow leaderboard modifying, echo = FALSE}
...
```{r README - Force submission to leaderboard, echo = FALSE}
...
```{r README - Has registration, echo = FALSE}
...
```{r README - Start date, echo = FALSE}
if ( is.null(x = configuration$start_date) ) {
    configurationYaml$start_date <- readline(prompt = paste("Start_Date (format : YYYY-MM-DD) [default = ", Sys.Date( ), "] : ", sep = "") )
}
if ( configurationYaml$start_date == "" ) {
    configurationYaml$start_date <- as.character(x = Sys.Date( ) )
}
print(x = configurationYaml$start_date)
```

- `end_date` : YYYY-MM-DD. No end date if left blank
```{r README - End date, echo = FALSE}
if ( is.null(x = configuration$end_date) ) {
    configurationYaml$end_date <- readline(prompt = paste("End_Date (format : YYYY-MM-DD) [default = ", Sys.Date( ) + 365, "] : ", sep = "") )
}
if ( configurationYaml$end_date == "" ) {
    configurationYaml$end_date <- as.character(x = Sys.Date( ) + 365)
}
...
```{r README - Enable detailed results, echo = FALSE}
...
- `admin_names` :  Comma separated list of Codalab IDs of co-organizers It is called admins in the editor.
```{r README - Admin names, echo = FALSE}
...
- `html` :
  ...
  ```{r README - Html evaluation, echo = FALSE}
  ...
  ```{r README - Html terms, echo = FALSE}
  ...
  ```{r README - Additional html pages, echo = FALSE}
...
### 2. Some properties of competitions cannot be specified in the YAML file. However, they are available from the editor.
...
### 3. Phase properties using the YAML file

- `phases` :
  ```{r README - Phases, echo = FALSE}
  ...
  for ( phase_idx in seq_len(length.out = configuration$phase_number ) ) {
      ...
      if ( is.null(x = configuration[[ paste("phases", phase_idx, "description", sep = "_" ) ]] ) ) {
          configurationYaml$phases[[ phase_idx_c ]]$description <- readline(prompt = "Description (format : string) [default = TEMPLATE : phase description.] : ")
      }
      if ( configurationYaml$phases[[ phase_idx_c ]]$description == "" ) {
          configurationYaml$phases[[ phase_idx_c ]]$description <- "TEMPLATE : phase description."
      }
      print(x = configurationYaml$phases[[ phase_idx_c ]]$description)

      cat("  - `start_date` : YYYY-MM-DD, Phase start (ends when new phase begins)\n")
      if ( is.null(x = configuration[[ paste("phases", phase_idx, "start_date", sep = "_" ) ]] ) ) {
          configurationYaml$phases[[ phase_idx_c ]]$start_date <- readline(prompt = paste("Start date (format : YYYY-MM-DD) [default : ", Sys.Date( ), "] : ", sep = "") )
      }
      if ( configurationYaml$phases[[ phase_idx_c ]]$start_date == "" ) {
          configurationYaml$phases[[ phase_idx_c ]]$start_date <- as.character(x = Sys.Date( ) )
      }
      ...
      if ( is.null(x = configuration[[ paste("phases", phase_idx, "public_data", sep = "_" ) ]] ) ) {
          configurationYaml$phases[[ phase_idx_c ]]$public_data <- readline(prompt = "Public data (format : filename with .zip extension, if 'no' the entry is removed from the configuration) [default : public_data.zip] : ")
      }
      if ( configurationYaml$phases[[ phase_idx_c ]]$public_data == "" ) {
          configurationYaml$phases[[ phase_idx_c ]]$public_data <- "public_data.zip"
      }
      ...
      if ( is.null(x = configuration[[ paste("phases", phase_idx, "input_data", sep = "_" ) ]] ) ) {
          configurationYaml$phases[[ phase_idx_c ]]$input_data <- readline(prompt = "Input data (format : filename with .zip extension (e.g. 'input_data.zip'), if 'no' the entry is removed from the configuration) [default : no] : ")
      }
      if ( configurationYaml$phases[[ phase_idx_c ]]$input_data == "" ) {
          configurationYaml$phases[[ phase_idx_c ]]$input_data <- "no"
      }
      ...
      if ( is.null(x = configuration[[ paste("phases", phase_idx, "ingestion_program", sep = "_" ) ]] ) ) {
          configurationYaml$phases[[ phase_idx_c ]]$ingestion_program <- readline(prompt = "Ingestion program (format : filename with .zip extension (e.g. ingestion_program.zip), if 'no' the entry is removed from the configuration) [default : no] : ")
      }
      if ( configurationYaml$phases[[ phase_idx_c ]]$ingestion_program == "" ) {
          configurationYaml$phases[[ phase_idx_c ]]$ingestion_program <- "no"
      }
...
### 4. Some phase properties cannot be specified in the YAML file. However, they are available from the editor:
...
### 5. Leaderboard properties using the YAML file

- `leaderboard` :
  ```{r README - Leaderboard, echo = FALSE}
...
### 6. Leaderboard features accessible only using the online editor
...
## IV. Generation of the Yaml configuration file

```{r README - Generation Yaml file, include = FALSE, cache = FALSE}
yaml::write_yaml(
          x            = configurationYaml
        , file         = paste(configuration$name, .Platform$file.sep, "competition.yaml", sep = "")
        , fileEncoding = "UTF-8"
      )
```

## V. Generation of the dataset

```{r README - Generation datasets, include = FALSE}
...
if ( generateData ) {
    rmarkdown::render(input = "generate_data.Rmd")
} else {
...
## VI. Generation of the scoring program (and optionally of the ingestion program)
...
```{r README - Generation scoring or ingestion programs, include = FALSE}
message <- readline(prompt = "Please, generate all the scoring programs and optionnaly the ingestion programs that are required, and then press 'ENTER'.")
remove(list = "message")
```

## VII. Generation of the html files

### 1. Overview

The file `r configurationYaml$html$overview` is required for the tab 'Overview'.
```{r README - Generation html overview, include = FALSE}
...
```{r README - Generation html evaluation, include = FALSE}
...
```{r README - Generation html terms, include = FALSE}
...
```{r README - Generation html data, include = FALSE}
...
### 5. Additional html pages

```{r README - Generation additional html pages, echo = FALSE}
for ( additional_page_idx in seq_len(length.out = configuration$html_additional_page_number ) ) {
...
## VIII. Generation of the phases

```{r README - Generation phases, echo = FALSE}
for ( phase_idx in seq_len(length.out = configuration$phase_number ) ) {
    phase_idx_c <- as.character(x = phase_idx)
    cat( paste("Phase ", phase_idx, " :\n", sep = "") )
    ...
    if ( !is.null(x = configurationYaml$phases[[ phase_idx_c ]]$public_data) ) {
        ...
        if ( !useFile ) {
            files <- readline(prompt = paste("Phase ", phase_idx, " : public data : files to add inside the zip file '", configurationYaml$phases[[ phase_idx_c ]]$public_data, "' (format : comma separated filenames) [default : data.rds] : ", sep = "") )
            if ( files == "" ) {
                files <- "data.rds"
            }
        ...
        for ( file in unzip(zipName, list = TRUE)$Name ) {
            cat( paste("  |-> ", file, "\n", sep = "") )
        }
    ...
    if ( !is.null(x = configurationYaml$phases[[ phase_idx_c ]]$input_data) ) {
        ...
        if ( !useFile ) {
            files <- readline(prompt = paste("Phase ", phase_idx, " : input data : files to add inside the zip file '", configurationYaml$phases[[ phase_idx_c ]]$input_data, "' (format : comma separated filenames) [default : data_input.rds] : ", sep = "") )
            if ( files == "" ) {
                files <- "data_input.rds"
            }
            files <- unlist(x = strsplit(x = files, split = ",") )
            zip(
                zipfile = zipName
    ...
    if ( !useFile ) {
        files <- readline(prompt = paste("Phase ", phase_idx, " : reference data : files to add inside the zip file '", configurationYaml$phases[[ phase_idx_c ]]$reference_data, "' (format : comma separated filenames) [default : data_reference.rds] : ", sep = "") )
            if ( files == "" ) {
                files <- "data_reference.rds"
            }
        files <- unlist(x = strsplit(x = files, split = ",") )
        zip(
            zipfile = zipName
    ...
    if ( !is.null(x = configurationYaml$phases[[ phase_idx_c ]]$starting_kit) ) {
        ...
        if ( !useFile ) {
            filesTemp <- sapply(
                X = c(unlist(x = configurationYaml$html), "starting_kit.Rmd", "generate_result.Rmd")
              , FUN = function(x) {
                  rmd <- sub(pattern = ".html", replacement = ".Rmd", x = x)
                  if ( file.exists( rmd ) ) {
                      rmd
                  } else {
                      x
            ...
            if ( !is.null(x = configurationYaml$phases[[ phase_idx_c ]]$public_data) ) {
                filesTemp <- c(filesTemp, paste(configuration$name, .Platform$file.sep, configurationYaml$phases[[ phase_idx_c ]]$public_data, sep = "") )
            }
...
## IX. Generation of the Codalab bundle
    
```{r README - Generation bundle, echo = FALSE, cache = FALSE}
bundleFiles <- c(
...
for ( file in unzip(bundlePath, list = TRUE)$Name ) {
    cat( paste("  |-> ", file, "\n", sep = "") )
}
...
## X. Rendering of a web site to previsualize the competition

It gives you the possibility to visualize your competition with a minimalist website which looks like the Codalab platform.
`rmarkdown::render_site(input = "README.Rmd")`


# Session Information

```{r README - Session information, results="verbatim"}
